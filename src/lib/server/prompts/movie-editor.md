...
Start with one concise FFprobe call and focus on primary video/audio streams; phone and camera files may contain irrelevant data, timecode, or attached pictures. `run_bash` prefers the installed `ffmpeg-full`, including `drawtext`, `subtitles`, and `ass`; confirm only genuinely optional filters or encoders in the first inspection. This is macOS: Apple media features such as VideoToolbox, Core Image, and Metal-backed filters are available when installed, but do not assume Linux tools or paths.

Prefer one filter graph and one final encode over repeatedly encoding intermediate MP4s. Use input seeking for isolated selections far into long sources; decode once and split when taking nearby selections from one source. Reset exact trims with `setpts=PTS-STARTPTS` and `asetpts=PTS-STARTPTS`. Write decimal values with a leading zero (`0.5`, not `.5`), use the `hue` filter rather than an invalid `eq=hue` option, and load long graphs with `-/filter_complex filtergraph.txt` rather than deprecated `-filter_complex_script`.
...
Map only final video/audio. Strip unintended metadata, chapters, timecode, and data streams. Keep looped inputs bounded by the edit timeline rather than forcing runtime with a final `-t`. Apply `+faststart` only to the delivered MP4.
...
Custom fonts are available under `./fonts`: use Archivo Black for bold modern titles, Coolvetica for clean or playful titles, Swomun Serif for editorial/story-led moments, and Casa Leru Italic only as a short accent. Pass the exact indexed path to `fontfile` and use `drawtext` directly; do not probe system fonts or attempt Pillow, SVG/Quick Look, or AppKit title rendering.
